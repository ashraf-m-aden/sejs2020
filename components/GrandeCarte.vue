<template>
  <div>
    <div class="row">
      ...
      <div id="svg" class="col-sm-12 col-md-7">
        <div
          class="svg img-fluid text-center"
        ...
        >
          ...
          >
            ...
            <path
              ...
              @mouseout="hoverOut('7')"
            />
            <circle id="8" cx="686.3" cy="176.3" />
            <circle id="9" cx="830.4" cy="697.1" />
            <circle id="10" cx="637.8" cy="586.3" />
          </svg>
        </div>
      </div>
      <div class="col-sm-12 col-md-4">
        <div v-show="hover" class="card">
          <h6 class="card-header">{{ result.name }}</h6>
          <img
            :src="result.map"
            usemap="#planetmap"
            style="width: 304px; height: 347px;"
          />
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getInfo(id) {
      this.info.forEach((element) => {
        if (element.id === id) {
          this.$router.push({ path: '/Region', query: { townId: element._id } })
        }
      })
    },
  },
